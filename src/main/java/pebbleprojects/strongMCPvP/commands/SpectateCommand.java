    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        TaskHandler.INSTANCE.runAsync(() -> {
            ...
            if (CombatLogHandler.INSTANCE.isInCombatLog(player)) {
                MessageHandler.INSTANCE.sendMessage(player, "spectate.failed.combat-log", null);
                return;
            }
            ...
            if (GameHandler.INSTANCE.addDelay(player.getUniqueId(), 2)) {
                ...
                MessageHandler.INSTANCE.sendMessage(player, "spectate.success.disable", null);
                return;
            }

            MessageHandler.INSTANCE.sendMessage(player, "spectate.failed.delay", null);
        });
        return false;
    }
}
